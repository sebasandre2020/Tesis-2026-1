...
\begin{itemize}
    ...
    \item Implementación y comparación de controladores PID, Fuzzy y LQR.
...
Esta arquitectura permite implementar distintas estrategias de control sobre una misma estructura física, facilitando la comparación entre controladores PID, Fuzzy y LQR. Asimismo, el uso de una configuración de lazo cerrado permite que el sistema responda de manera automática frente a variaciones en la ocupación del ambiente o en la concentración de CO$_2$, favoreciendo una operación más estable y adaptable que un esquema de activación basado únicamente en umbrales fijos.
...
A partir de esta formulación, es posible simular distintos escenarios de operación y analizar la respuesta del sistema ante cambios en la ocupación o en las condiciones de ventilación. Asimismo, este modelo proporciona una base adecuada para la implementación y comparación de controladores PID, Fuzzy y LQR, al permitir evaluar la evolución de la concentración de CO$_2$ frente a diferentes acciones de control \cite{delwati_2018, bordin_2025}.
...
\subsection{Diseño e implementación de controladores PID, Fuzzy y LQR}
...
Para la simulación, la Integral del error también puede expresarse en ppm/min, con el objetivo de representar mejor la velocidad de cambio de la concentración:

\begin{equation}
    \Delta e_{min}(k)=\frac{e(k)-e(k-1)}{\Delta t}\cdot 60
\end{equation}
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{|p{3cm}|p{3cm}|p{4cm}|p{5cm}|}
            ...
            Variación del error & \([-30,30]\) ppm/min & Baja, Media, Alta           & Indica si el error disminuye, permanece casi estable o aumenta.           \\ \hline
...
La base de reglas se definió considerando que, mientras mayor sea el error de CO$_2$ y mayor sea su tendencia de incremento, mayor debe ser la acción de ventilación. Por el contrario, si el error es bajo y la concentración disminuye, la potencia puede reducirse.
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{|c|c|c|c|}
            \hline
            \textbf{Error / Derivada del error} & \textbf{Baja der.} & \textbf{Media der.} & \textbf{Alta der.} \\ \hline
...
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Error / Integral del error} & \textbf{Baja der.} & \textbf{Media der.} & \textbf{Alta der.} \\ \hline
\textbf{Bajo PPM} & Cero POT & Baja POT & Baja POT \\ \hline
\textbf{Medio PPM} & Baja POT & Media POT & Media POT \\ \hline
\textbf{Alto PPM} & Media POT & Media POT & Alta POT \\ \hline
\textbf{Muy Alto PPM} & Alta POT & Alta POT & Alta POT \\ \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/capitulo3/fuzzy_respuesta_co2.png}
    \caption{Respuesta de concentración de CO$_2$ con controlador Fuzzy.}
    \label{fig:fuzzy_respuesta_co2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/capitulo3/fuzzy_pwm.png}
    \caption{Esfuerzo de control del controlador Fuzzy con PWM base.}
    \label{fig:fuzzy_pwm}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/capitulo3/fuzzy_caudal.png}
    \caption{Caudal efectivo total generado por el sistema de ventilación.}
    \label{fig:fuzzy_caudal}
\end{figure}
