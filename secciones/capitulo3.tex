...
\begin{itemize}
    \item \textbf{Interfaz Gateway - Nube (Ingesta de Datos):} El gateway (basado en ESP32) funciona como el puente entre el entorno LoRa local y la infraestructura en Azure. La comunicación se realiza mediante peticiones HTTP (REST) sobre una conexión segura (HTTPS).
    \item \textbf{Compatibilidad de interfaces:} El microcontrolador del gateway debe interconectarse de manera eficiente con el módulo transceptor LoRa y disponer de hardware para conexión a internet (Wi-Fi).
    \item \textbf{Capacidad de procesamiento:} El gateway debe contar con los recursos computacionales suficientes para recibir las tramas LoRa, empaquetar los datos recopilados de los nodos (ID, CO\(_2\), timestamp) en formato JSON y enviarlos mediante un método POST hacia el endpoint de la nube.
    \item \textbf{Requerimientos de alimentación:} El dispositivo deberá operar de manera continua mediante una fuente de alimentación conectada a la red eléctrica del aula, garantizando un voltaje estable.
    \item \textbf{Recepción en Backend (Capa de Ingesta):} El backend debe estar configurado para recibir el HTTP POST, validar la trama JSON, evaluar los umbrales de alerta de CO\(_2\) y ejecutar la orden de guardado.
    \item \textbf{Almacenamiento y alertas:} Los datos recibidos deben almacenarse en la base de datos Azure Cosmos DB (NoSQL) para su consulta histórica. Si las lecturas superan el umbral de CO\(_2\), el backend activará el sistema de alertas.
\end{itemize}

\subsubsection{Requisitos de Almacenamiento y Persistencia de Datos}

\begin{itemize}
    \item Los datos de telemetría de los sensores deben almacenarse en Azure Cosmos DB (NoSQL), justificado por su alta eficiencia y escalabilidad para almacenar telemetría rápida y no estructurada típica de sensores IoT.
    \item La base de datos debe registrar el identificador del nodo (ID), la marca de tiempo (timestamp), la ubicación y el nivel de CO\(_2\) de cada lectura.
    \item Los datos históricos deben poder ser visualizados por el usuario a través de gráficos en la interfaz de detalle de cada sensor mediante consultas a la API del backend.
\end{itemize}
...
\begin{table}[h!]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{|p{3.2cm}|p{2.5cm}|p{2.5cm}|p{2.5cm}|p{2.8cm}|}
            \hline
            \textbf{Arquitectura}              & \textbf{Escalabilidad}                                                                               & \textbf{Mantenibilidad}                                                                              & \textbf{Manejo de Datos IoT}                                                                          & \textbf{Capacidad de Despliegue}                                                                          \\ \hline
            \textbf{Monolítica (MVC o Capas)}  & Moderada: frontend y backend acoplados dificultan escalar componentes por separado                   & Alta al inicio, pero compleja si el proyecto crece y requiere tecnologías distintas                  & Limitada: el procesamiento sincrónico puede generar cuellos de botella con alta telemetría            & Despliegue en un único servidor, limitando la adopción de servicios Serverless o PaaS en la nube          \\ \hline
            \textbf{Microservicios}            & Muy alta: ideal para aplicaciones inmensas, permite escalar y desplegar servicios independientes     & Baja: requiere alta experiencia para orquestar contenedores, gestionar redes y despliegues complejos & Muy buena: ingesta y consultas pueden separarse en microservicios dedicados                           & Requiere orquestadores como Kubernetes, elevando significativamente el costo y la complejidad operativa   \\ \hline
            \textbf{Client-Server Desacoplada} & Alta: permite separar los recursos de ingesta de datos de los recursos de visualización del frontend & Alta: frontend y backend evolucionan de forma independiente usando las tecnologías más aptas         & Excelente: permite integrar un pipeline de datos optimizado (Ingesta, Almacenamiento, API Serverless) & Óptima para la nube moderna, permitiendo usar Container Apps para backend y Static Web Apps para frontend \\ \hline
        \end{tabular}
    }
\end{table}

Para este trabajo de investigación, se determinó que la \textbf{Arquitectura Client-Server Desacoplada} es la idónea. Esta arquitectura rige el desarrollo separando estrictamente el frontend del backend, lo que permite un despliegue optimizado en la nube de Azure y un pipeline de datos robusto para el procesamiento de telemetría IoT.

Bajo este modelo, la solución se estructura en tres capas principales de despliegue en la nube:
\begin{itemize}
    \item \textbf{Capa de Ingesta y Lógica:} Un backend ligero desarrollado en C\# (.NET Core) alojado en Azure Container Apps. Su función es recibir el tráfico HTTP POST desde el gateway, validar la trama JSON, evaluar los umbrales de alerta de CO\(_2\) y ejecutar la orden de guardado.
    \item \textbf{Capa de Almacenamiento:} Se utiliza Azure Cosmos DB (NoSQL), justificado por su alta eficiencia y baja latencia para almacenar series temporales y telemetría rápida y no estructurada típica de sensores IoT.
    \item \textbf{Capa de Visualización:} Un panel web alojado en Azure Static Web Apps. Utiliza Managed Functions (API Serverless) para abstraer la consulta del historial almacenado en Cosmos DB y alimentar el frontend de forma escalable y segura.
\end{itemize}
\FloatBarrier
...
Dado que el trabajo de investigación requiere una interfaz dinámica capaz de procesar telemetría en tiempo real, se considera que React es la opción más adecuada. Su ecosistema facilita la integración con APIs y la gestión de estados complejos en aplicaciones Single Page Application (SPA).

En el desarrollo del frontend (React.js), se aplican patrones de diseño clave para asegurar el desacoplamiento y la reactividad de la interfaz:
\begin{itemize}
    \item \textbf{Smart vs. Dumb Components (Contenedor / Presentación):} Se separan estrictamente los componentes que manejan la lógica de negocio y llamadas a la API (Smart) de aquellos dedicados exclusivamente a la renderización gráfica de los datos (Dumb).
    \item \textbf{Patrón Observer:} Implementado a través de librerías como React Query para gestionar el \textit{polling} o las suscripciones. Esto permite que la interfaz escuche y se actualice en tiempo real ante nuevos registros de CO\(_2\), manteniendo los datos sincronizados con la nube.
    \item \textbf{Provider Pattern (Context API):} Utilizado para el manejo eficiente del estado global de la aplicación. Facilita la propagación de datos transversales, como la sesión del usuario o el nodo sensor actualmente seleccionado, sin necesidad de perforación de propiedades (\textit{prop drilling}).
\end{itemize}
...
\begin{landscape}
    \begin{table}[h!]
        ...
        \caption{Patrones de Diseño Utilizados en el Backend \cite{design_patterns}}
        ...
        \begin{tabular}{|p{2cm}|p{3cm}|p{3cm}|p{5cm}|p{2cm}|}
            ...
            Abstracción que maneja las operaciones de acceso a datos aislando la capa de persistencia.                                     &
            Abstrae y centraliza todas las consultas e inserciones hacia Azure Cosmos DB, separando la base de datos de los controladores. &
            \begin{itemize}[leftmargin=*, nosep]
                \item Desacopla la lógica de negocio del proveedor NoSQL.
                \item Centraliza las consultas, evitando duplicidad.
                \item Facilita \textit{mocks} para pruebas unitarias.
            \end{itemize}                                                                  &
            Capa de Ingesta (Repositorios)                                                                                                                                                                                                                      \\ \hline

            \textbf{Inyección de Dependencias}                                                                                             &
            Patrón donde los objetos reciben sus dependencias en lugar de instanciarlas directamente.                                      &
            Gestiona y suministra de forma eficiente los recursos (ej. clientes Cosmos DB) en los controladores de la API.                 &
            \begin{itemize}[leftmargin=*, nosep]
                \item Garantiza un código altamente testeable y modular.
                \item Optimiza el ciclo de vida de los servicios.
                \item Reduce el acoplamiento fuerte.
            \end{itemize}                                                                   &
            Capa de Ingesta (Controladores)                                                                                                                                                                                                                     \\ \hline

            \textbf{DTO (Data Transfer Object)}                                                                                            &
            Objeto que encapsula datos para la transferencia en red sin exponer modelos de dominio.                                        &
            Mapea la trama JSON del POST del gateway a C\#, validando campos como ID, nivel de CO\(_2\) y timestamp.                       &
            \begin{itemize}[leftmargin=*, nosep]
                \item Previene vulnerabilidades de sobre-publicación.
                \item Define contratos claros en la API HTTP.
            \end{itemize}                                                                      &
            Capa de Ingesta                                                                                                                                                                                                                                     \\ \hline
        \end{tabular}
    \end{table}
...
\subsubsection{Desarrollo de REST API y Pipeline en la Nube}
\begin{figure}
    \centering
...
Como se ilustra en la Figura \ref{fig:df_backend}, el flujo de desarrollo del backend se adapta al nuevo pipeline de datos alojado en Azure. La implementación del backend (Capa de Ingesta), alojado como un contenedor ligero en Azure Container Apps, está construida en C\# (.NET Core). Su configuración inicial establece las políticas de seguridad para exponer un endpoint HTTPS público que recepciona la información.

El ciclo de procesamiento inicia cuando los controladores reciben el JSON vía petición \texttt{HTTP POST} desde el gateway ESP32. Tras la inyección de dependencias, los DTOs validan la estructura del mensaje. Posteriormente, la capa de servicios evalúa la lógica de negocio, verificando si el nivel de CO\(_2\) amerita la activación de alertas. Finalmente, mediante el \textit{Repository Pattern}, el servicio interactúa de forma directa y eficiente con Azure Cosmos DB (NoSQL) para registrar el evento histórico de la telemetría, reemplazando el uso de bases de datos relacionales tradicionales como MySQL.

Las pruebas unitarias y de integración aseguran la robustez de la lógica de evaluación y las consultas a Cosmos DB antes de empaquetar la aplicación en su contenedor Docker y ser desplegada a producción. El proceso de Quality Assessment verifica que todos los componentes cumplen con los estándares de calidad antes de finalizar el desarrollo.

\subsection{Diagrama de Arquitectura de Interconexión Cloud-Gateway}

A continuación, se presenta el diagrama de bloques que sintetiza la arquitectura de red y el flujo de datos del sistema, conectando el entorno físico con los recursos desplegados en la nube.

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[
            >=Stealth,
            box/.style={rectangle, draw=black, thick, fill=blue!5, text width=3.8cm, align=center, minimum height=1.6cm, rounded corners, font=\small},
            arrow/.style={->, thick},
            biarrow/.style={<->, thick}
        ]

        % Nodos
        \node[box] (gateway) at (0, 2) {\textbf{Gateway ESP32}\\(Origen físico)};
        \node[box] (container) at (6.5, 2) {\textbf{Azure Container Apps}\\(Backend / Lógica)};
        \node[box] (cosmos) at (12.5, 0) {\textbf{Azure Cosmos DB}\\(Base de Datos)};
        \node[box] (static) at (6.5, -2) {\textbf{Azure Static Web Apps}\\Managed Functions\\(API Serverless)};
        \node[box] (cliente) at (0, -2) {\textbf{Cliente Web / React.js}\\(Frontend)};

        % Flechas
        \draw[arrow] (gateway) -- node[above, font=\footnotesize] {HTTP POST (JSON)} (container);
        \draw[biarrow] (container.east) -- node[above right, font=\footnotesize, align=center, xshift=-0.3cm] {Escritura/\\Validación} (cosmos.150);
        \draw[arrow] (static.east) -- node[below right, font=\footnotesize, align=center, xshift=-0.3cm] {Lectura/\\Consulta} (cosmos.210);
        \draw[biarrow] (cliente) -- node[above, font=\footnotesize, align=center] {Petición HTTP\\/ Polling} (static);

    \end{tikzpicture}
    \caption{Diagrama de Arquitectura de Interconexión Cloud-Gateway.}
    \label{fig:arquitectura_nube}
\end{figure}

El diagrama de la Figura \ref{fig:arquitectura_nube} ilustra visualmente la Arquitectura Client-Server Desacoplada propuesta para el sistema de monitoreo. Como se observa, existe una separación estricta entre el flujo de ingesta de datos y el flujo de consumo. Por un lado, el Gateway ESP32 emite la telemetría hacia un backend dedicado exclusivamente a la recepción y evaluación (Azure Container Apps), el cual persiste la información en Cosmos DB validando el \textit{Repository Pattern}.

Por otro lado, la interacción con el usuario está completamente aislada de la lógica de ingesta. El Cliente Web desarrollado en React.js, aplicando los patrones \textit{Observer} y \textit{Smart vs. Dumb Components}, consume los datos históricos y las alertas interactuando únicamente con una API Serverless (Azure Static Web Apps), la cual se encarga de consultar a Cosmos DB de manera independiente. Esta división de responsabilidades valida de manera integral los patrones de diseño previamente mencionados y garantiza la escalabilidad del proyecto, evitando que un volumen masivo de ingesta de datos por parte de los sensores comprometa el rendimiento de la interfaz gráfica y viceversa.
